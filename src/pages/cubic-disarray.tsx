import { initCanvas, r90, plusOrMinus } from '../utils'
...
function draw(
  ...
  for (let i = size; i <= w - size; i += size) {
    for (let j = size; j <= w - size; j += size) {
      let rotateAmt =
        (((j / w) * Math.PI) / 90) * plusOrMinus() * Math.random() * (size / 5)
      let translateAmt = ((j / w) * plusOrMinus() * Math.random() * size) / 5
      ctx.save()
...
export default function () {
  ...
  const [nCubes, setNCubes] = useState(9)
